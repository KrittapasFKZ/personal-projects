...
register("command", () => {
    ...
    if (bzData && bzData[itemID]) {
        itemPrice = Number(bzData[itemID].sell * 1).toFixed(2)
    } else if (ahData && ahData[grItemID]) {
        itemPrice = Number(ahData[grItemID] * 1).toFixed(2)
    } else {
        itemPrice = 0
    };

    ChatLib.chat(`${Settings.prefix} &aItemPrice: &6${ShortNumber(itemPrice)}`)

}).setName("checkitemprice");