...
exports.run = async (bot, message, args) => {
    ...
    try {
        ...
        if (!args[0]) {
            return message.reply(`ไหนข้อความ มึงไม่บอกแล้วกูจะพูดอะไร`)
                .then(msg => {
                    setTimeout(() => {
                        if (msg) {
        ...
        if (config.tts_status == false && message.author.id != "605361556297089035") {
            return message.reply(`ตอนนี้คำสั่งโดนปิดใช้งานอดไปนะไอฟาย`)
                .then(msg => {
                    setTimeout(() => {
                        if (msg) {
                            msg.delete().catch(console.error);
        ...
        if (cooldowns.has(message.author.id)) {
            ...
            if (expirationTime && Date.now() < expirationTime) {
                ...
                return message.reply(`ใจเย็นไอเด็กเหี้ยรออีก ${timeLeft.toFixed(1)} วินะไอหน้าหี`)
                    .then(msg => {
                        setTimeout(() => {
                            if (msg) {
                                msg.delete().catch(console.error);
        ...
        if (Total_Queue <= 0 && getAudioQueue(message.author.id) == "IDLE") {
            Board.setAuthor({ name: `NozomiBot - Audio Queue`, iconURL: `${bot.user.displayAvatarURL()}` })
                .setDescription(`🎵 Playing TH TTS - ${textToSpeak}`)
                .setColor('#F5B041')
        } else {
            Board.setAuthor({ name: `NozomiBot - Audio Queue`, iconURL: `${bot.user.displayAvatarURL()}` })
        ...
        if (config.audio_queue_logs == false) {
            bot.BotLogs(message.guild.name, `\x1b[35m\x1b[90m[\x1b[37m${message.author.tag}\x1b[90m] \x1b[35mused TTS for \x1b[90m[\x1b[37m${new_text}\x1b[90m]`);
        };
