    ...
    try {
        ...
        if (Total_Queue <= 0 && getAudioQueue(message.author.id) == "IDLE") {
            Board.setAuthor({ name: `NozomiBot - Audio Queue`, iconURL: `${bot.user.displayAvatarURL()}` })
                .setDescription(`🎵 Playing Track - Megu Nigga (AUDIO_MP3)`)
                .setColor('#F5B041')
        } else {
            Board.setAuthor({ name: `NozomiBot - Audio Queue`, iconURL: `${bot.user.displayAvatarURL()}` })
        ...
        if (config.audio_queue_logs == false) {
            bot.BotLogs(message.guild.name, `\x1b[35mMegu Nigga has been used by \x1b[90m[\x1b[37m${message.author.tag}\x1b[90m]`);
        };
...
exports.help = {
    name: 'megu_nig',
    aliases: ['megu', 'meguna', 'meguna_nig', 'meguna_nigga']
};