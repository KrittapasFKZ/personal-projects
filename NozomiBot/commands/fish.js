...
const { fishing_ponds, talkedRecently } = require('./force_fish.js');;
...
exports.run = async (bot, message, args) => {
    ...
    try {
        ...
        if (fs.existsSync(`./database/variables/${message.guild.id}.json`)) {
            ...
            allow_ID = jsonData.channel.fishing
        };

        if (message.channel.id == allow_ID) { } else {
            const EventEmbed_Failed = new EmbedBuilder()
                .setTitle(`Interaction Denied!`)
                .setDescription(`❌ ไม่สามารถตกปลาใน Channel นี้ได้!`)
                .setColor('#FF0000')
                .setTimestamp()
        ...
        if (config.fishing_status == false && message.author.id != "605361556297089035") {
            const EventEmbed_Failed = new EmbedBuilder()
                .setTitle(`Interaction Denied!`)
                .setDescription(`❌ ตอนนี้ระบบตกปลาถูกปิดใช้งาน!`)
                .setColor('#FF0000')
                .setTimestamp()
        ...
        if (users[userId]) {
            ...
            const EventEmbed_Failed = new EmbedBuilder()
                ...
                .setDescription(`❌ คุณมี Bait ไม่เพียงพอ!`)
                .setColor('#FF0000')
                .setTimestamp()
            ...
            bot.BotLogs(message.guild.name, `\x1b[36m\x1b[37m"${message.author.tag}" \x1b[36mstarted Fishing Simulator!`);
            ...
            async function StopFishing() {
                ...
                fs.writeFileSync(`./database/fishing_ponds.json`, JSON.stringify(Array.from(fishing_ponds.entries()), null, 4));
                MainEmbed.delete();
                const RecordsEmbed = await message.channel.send({ content: mention, embeds: [StopEmbed] });
                setTimeout(async () => {
                    RecordsEmbed.delete();
                }, 6000);
                bot.BotLogs(message.guild.name, `\x1b[36m\x1b[37m"${message.author.tag}" \x1b[36mstopped Fishing Simulator!`);
            };
