...
exports.run = async (bot, message, args) => {
    ...
    try {
        ...
        const members = voiceChannel.members.filter(member => !member.user.bot && member.id !== message.author.id && member.id !== "887531368836370483");
        ...
        if (Total_Queue <= 0 && getAudioQueue(message.author.id) == "IDLE") {
            Board.setAuthor({ name: `NozomiBot - Audio Queue`, iconURL: `${bot.user.displayAvatarURL()}` })
                .setDescription(`🎵 Playing Track - Gojo Satoru Murasaki (AUDIO_MP3)`)
                .setColor('#F5B041')
        } else {
            Board.setAuthor({ name: `NozomiBot - Audio Queue`, iconURL: `${bot.user.displayAvatarURL()}` })
        ...
        if (config.audio_queue_logs == false) {
            bot.BotLogs(message.guild.name, `\x1b[35mGojo Satoru Murasaki has been used by \x1b[90m[\x1b[37m${message.author.tag}\x1b[90m]`);
        };

        let queue_constructor = {
            uuid: `${generateUUID()}`,
            name: "Gojo Satoru Murasaki",
            file: "murasaki.mp3",
            type: "MURASAKI",
            guild: message.guild,
