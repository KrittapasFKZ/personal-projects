...
public class NozomiAddonClient implements ClientModInitializer{
    ...
    public void onInitializeClient() {
        ...
        LOGGER.info("NozomiAddon initialized for Minecraft 1.21.x!");
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("NozomiAddon")
                    .executes(context -> {
                        new Thread(() -> {
                            try {
                                Thread.sleep(100);
                            ...
                            MinecraftClient.getInstance().execute(() -> {
                                if (ClickGUI.displayMode.getMode().equals("V2")) {
                                    MinecraftClient.getInstance().setScreen(new ModMenuGrid());
                                } else {
                                    MinecraftClient.getInstance().setScreen(new ModMenuList());
                                }
                            });
                        }, "na-open-delay").start();
                        return 1;
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("nagetworld")
                    .executes(context -> {
                        WorldUtils.sendWorldToPlayer(MinecraftClient.getInstance().player);
                        return 1;
                    })
            );
        });
