...
public class AutoClick implements FeatureManager.Feature {
    ...
    public void initClient() {
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (AutoClick.listOnly.isEnabled()) {
                if (isHoldingAllowedItem(client, AutoClick.allowlist) && client.options.attackKey.isPressed() && nowMillis >= nextLeftClick) {
            ...
            } else if (AutoClick.enableLeftClick.isEnabled() && client.options.attackKey.isPressed() && nowMillis >= nextLeftClick) {
            ...
            if (AutoClick.bow_listOnly.isEnabled()) {
                if (isHoldingAllowedItem(client, AutoClick.bow_allowlist) && client.options.useKey.isPressed() && nowMillis >= nextLeftClick) {
                    double randomCps = AutoClick.bow_cps.getRandomValue();
                    if (randomCps <= 0) randomCps = 1;
                    nextLeftClick = nowMillis + (long) ((1000.0 / randomCps) + ((Math.random() - 0.5) * 60.0));
                    simulateLeftClick(client);
                }
            } else if (AutoClick.enableRightClick.isEnabled() && client.options.useKey.isPressed() && nowMillis >= nextRightClick) {
    ...
    private boolean isHoldingAllowedItem(MinecraftClient client, MultiSelectSetting targetList) {
        if (client.player == null) return false;
        ItemStack mainHand = client.player.getMainHandStack();
        if (mainHand == null || mainHand.isEmpty()) return false;
        Text name = mainHand.getName();
    ...
    private boolean isHoldingIgnoredItem(MinecraftClient client) {
    ...
    private void simulateRightClick(MinecraftClient client) {
