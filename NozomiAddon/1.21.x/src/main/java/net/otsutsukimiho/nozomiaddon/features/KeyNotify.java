    ...
    public void initClient() {
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
            ...
            if (msg.contains("has obtained Blood Key!")) {
                MinecraftClient.getInstance().player.sendMessage(Text.literal("§d§lNA §f§l» §c§lBlood Key Picked!"),false);
                ...
                MinecraftClient.getInstance().inGameHud.setSubtitle(formattedText);
                MinecraftClient.getInstance().inGameHud.setTitleTicks(0, 30, 10);
            }
            if (msg.contains("has obtained Wither Key!")) {
                MinecraftClient.getInstance().player.sendMessage(Text.literal("§d§lNA §f§l» §8§lWither Key Picked!"),false);
                customSound.playTestSound();
