...
public class CustomCommand {
    ...
    public static void init() {
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("nacmd")
                            ...
                            .then(ClientCommandManager.argument("trigger", StringArgumentType.word())
                                    .then(ClientCommandManager.argument("command", StringArgumentType.greedyString())
                                            .executes(context -> {
                                                ...
                                                context.getSource().sendFeedback(Text.literal("§d§lNA §f§l» §aAdded §e/" + trigger + " §a-> §e/" + command));
                                                return 1;
                                            })
                                    )
                            )
                            ...
                            .then(ClientCommandManager.argument("trigger", StringArgumentType.word())
                                    .executes(context -> {
                                        ...
                                        if (shortcuts.remove(trigger) != null) {
                                            save();
                                            context.getSource().sendFeedback(Text.literal("§d§lNA §f§l» §aRemoved §e/" + trigger));
                                        } else {
                                            context.getSource().sendFeedback(Text.literal("§d§lNA §f§l» §cUnknown CMD."));
                                        }
                                        return 1;
                                    })
                            )
                    ...
                    .then(ClientCommandManager.literal("list")
                            .executes(context -> {
                                context.getSource().sendFeedback(Text.literal("§d§lNA §f§l» §aCommand Lists"));
                                if (shortcuts.isEmpty()) {
                                    context.getSource().sendFeedback(Text.literal("§7(No commands saved)"));
                                } else {
                                    shortcuts.forEach((k, v) -> {
                                        context.getSource().sendFeedback(Text.literal("§d§lNA §f§l» §e/" + k + " §a-> §e/" + v));
                                    });
                                }
                                return 1;
                            })
                    ...
                    .then(ClientCommandManager.literal("reload")
                            .executes(context -> {
                                load();
                                context.getSource().sendFeedback(Text.literal("§d§lNA §f§l» §aConfiguration reloaded!"));
                                return 1;
                            })
                    )
            );
    ...
    private static boolean onCommand(String command) {
        if (shortcuts.containsKey(command)) {
            ...
            if (MinecraftClient.getInstance().player != null) {
                MinecraftClient.getInstance().player.networkHandler.sendChatCommand(realCommand);
            }
            return false;
        }
        return true;
    ...
    private static void save() {
        try {
            ...
            if (MinecraftClient.getInstance().player != null) {
                MinecraftClient.getInstance().player.sendMessage(Text.literal("§cFailed to save commands!"), false);
            }
        }
    }
