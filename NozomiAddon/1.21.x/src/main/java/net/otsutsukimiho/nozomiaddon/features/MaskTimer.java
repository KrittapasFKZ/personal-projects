...
public class MaskTimer extends DraggableHudElement implements FeatureManager.Feature {
    ...
    public MaskTimer() {
        ...
        this.spiritItem = HeadUtils.getSkull(SPIRIT_TEXTURE);
        this.bonzoItem = HeadUtils.getSkull(BONZO_TEXTURE);
        this.phoenixItem = HeadUtils.getSkull(PHOENIX_TEXTURE);
    }
    ...
    private final ItemStack spiritItem;
    private final ItemStack bonzoItem;
    private final ItemStack phoenixItem;
    ...
    private void showMaskTitle(String text) {
        MinecraftClient.getInstance().execute(() -> {
            if (MinecraftClient.getInstance().player != null) {
                MinecraftClient.getInstance().player.sendMessage(Text.of("§d§lNA §f§l» " + text), false);
                ...
                MinecraftClient.getInstance().inGameHud.setSubtitle(Text.literal(text));
                MinecraftClient.getInstance().inGameHud.setTitleTicks(0,40,0);
            }
        });
    }

    private void drawMaskWithItem(DrawContext ctx, ItemStack item, String name, float cooldown, int y) {
        ...
        ctx.drawText(MinecraftClient.getInstance().textRenderer, Text.literal(status), textX, textY, 0xFFFFFFFF, true);
    }
}