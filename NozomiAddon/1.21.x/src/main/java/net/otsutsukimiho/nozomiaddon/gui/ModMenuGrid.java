...
public class ModMenuGrid extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int i = 0; i < itemsToDisplay.size(); i++) {
            ...
            float scale = 1.5f;
            context.getMatrices().translate(cardX + currentCardWidth / 2.0f, cardY + 20.0f);
            context.getMatrices().scale(scale, scale);
            context.drawItem(iconStack, -8, -8);
            context.getMatrices().popMatrix();

            String shortName = displayKey;
            if (textRenderer.getWidth(shortName) > currentCardWidth - 10) {
            ...
            context.drawCenteredTextWithShadow(textRenderer, shortName, cardX + currentCardWidth / 2, cardY + 38, titleColor);
            ...
            if (hasSettings) {
                ...
                context.drawCenteredTextWithShadow(textRenderer, "Settings", cardX + currentCardWidth / 2, optionsY + 4, optTextColor);
            }
            ...
            if (currentToggle > 0.01f) {
                ...
                int tw = textRenderer.getWidth("§lENABLED");
                context.drawText(textRenderer, "§lENABLED", cardX + currentCardWidth / 2 - tw / 2, toggleY + 6, enabledColor, false);
            }
            if (currentToggle < 0.99f) {
                int disabledAlpha = (int)((1.0f - currentToggle) * 255);
                int disabledColor = (disabledAlpha << 24) | 0x777777;
                int tw = textRenderer.getWidth("§lDISABLED");
                context.drawText(textRenderer, "§lDISABLED", cardX + currentCardWidth / 2 - tw / 2, toggleY + 6, disabledColor, true);
            }
