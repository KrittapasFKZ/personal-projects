...
public class DungeonRunOverview extends DraggableHudElement implements FeatureManager.Feature {
    ...
    public void initClient() {
        ...
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
            ...
            if (msg.contains("[BOSS] The Watcher: That will be enough for now.")) {
                client.player.sendMessage(Text.literal("§d§lNA §f§l» §cBlood Ready!"),false);
            }
            ...
            if (msg.contains("[BOSS] Storm: Pathetic Maxor, just like expected.")) {
                if (dungeonStep == 3) {
                    dungeonStep = 4;
                    client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §aMaxor Done: %.2fs", (ticks.getOrDefault("Maxor", 0F) / 20.0))),false);
                }
                zone = "Storm";
            }
            if (msg.contains("[BOSS] Goldor: Who dares trespass into my domain?")) {
                if (dungeonStep == 4) {
                    dungeonStep = 5;
                    client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §bStorm Done: %.2fs", (ticks.getOrDefault("Storm", 0F) / 20.0))),false);
                }
                zone = "Terminals";
            }
            if (msg.contains("The Core entrance is opening!")) {
                if (dungeonStep == 5) {
                    dungeonStep = 6;
                    client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §eTerminals Done: %.2fs", (ticks.getOrDefault("Terminals", 0F) / 20.0))),false);
                }
                zone = "Goldor";
            }
            if (msg.contains("[BOSS] Necron: You went further than any human before, congratulations.")) {
                if (dungeonStep == 6) {
                    dungeonStep = 7;
                    client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §7Goldor Done: %.2fs", (ticks.getOrDefault("Goldor", 0F) / 20.0))),false);
                }
                zone = "Necron";
            }
            if (msg.contains("[BOSS] Necron: All this, for nothing...")) {
                if (dungeonStep == 7) {
                    dungeonStep = 8;
                    client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §cNecron Done: %.2fs", (ticks.getOrDefault("Necron", 0F) / 20.0))),false);
                }
                zone = "Dragons";
            }
        });
    ...
    public static void tick () {
        ...
        if (bloodStart) {
            ...
            if (dungeonStep == 0) {
                dungeonStep = 1;
                client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §c§lBLOOD DOOR §chas been opened! §f(§a%s§f)", bloodOpened)),false);
            }
        }
        if (DUNGEON.watcherCleared != -1 && DUNGEON.bossEntry == -1) {
            ...
            if (dungeonStep == 1) {
                dungeonStep = 2;
                client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §cBlood Done! §f(took §a%.2fs§f)", (bloodTick / 20.0))),false);
            }
        }
        if (DUNGEON.bossEntry != -1) {
            ...
            if (dungeonStep == 2) {
                dungeonStep = 3;
                client.player.sendMessage(Text.literal(String.format("§d§lNA §f§l» §aBoss Entry: §a%s", getTime(bossEntry))),false);
    ...
    public void render(DrawContext ctx, RenderTickCounter tickCounter) {
        ...
        if (!DUNGEON.inDungeon) {
            if (EditHudScreen.isEditMode()) {
                ...
                ctx.drawText(MinecraftClient.getInstance().textRenderer, Text.literal(text), 0, y, 0xFFFFFFFF, true);
            }
        } else {
