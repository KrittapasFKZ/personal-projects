        ...
        ClientTickEvents.END_LEVEL_TICK.register(c -> {
            ...
            if (!complete) {
                if (cryptFound >= 5) {
                    ...
                    client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §cCrypts Done! §f(§a" + getTime(elapsedMs) + "§f)"));
            ...
            if (reminderSent <= 1) {
                ...
                if ((reminderSent == 0 && seconds >= CryptNotify.firstAnnounce.getValue() && seconds < (CryptNotify.firstAnnounce.getValue() + 10)) || (reminderSent == 1 && seconds >= CryptNotify.secondAnnounce.getValue() && seconds < (CryptNotify.secondAnnounce.getValue() + 10))) {
                    ...
                    client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §cNeed §e" + (5 - cryptFound) + " §cmore Crypts! §e#" + reminderSent));
