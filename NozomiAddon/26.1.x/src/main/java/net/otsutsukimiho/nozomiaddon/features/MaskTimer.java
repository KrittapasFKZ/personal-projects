    ...
    private void showMaskTitle(String text) {
        Minecraft.getInstance().execute(() -> {
            if (Minecraft.getInstance().player != null) {
                Minecraft.getInstance().gui.getChat().addClientSystemMessage(Component.nullToEmpty("§d§lNA §f§l» " + text));
                ...
                Minecraft.getInstance().gui.setSubtitle(Component.literal(text));
                Minecraft.getInstance().gui.setTimes(0,40,0);
            }
        });
    }
