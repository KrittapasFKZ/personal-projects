...
public class LeapMenu implements FeatureManager.Feature {
    ...
    public void initClient() {
                ...
                if (title.equals("Spirit Leap") || title.equals("Teleport to Player")) {
                    ...
                    ScreenMouseEvents.allowMouseClick(screen).register((screen2, click) -> {
                        ...
                        if (quadrant < players.size()) {
                            ...
                            if (target.isDead) {
                                if (client.player != null) client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §cThis player is dead!"));
                                return false;
                            }
                            ...
                            if (client.gameMode != null && client.player != null) {
                                ...
                                client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aLeaping to " + target.name + "!"));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (isOpen) {
                if (!(client.screen instanceof AbstractContainerScreen<?> handled) || (!handled.getTitle().getString().equals("Spirit Leap") && !handled.getTitle().getString().equals("Teleport to Player"))) {
                    isOpen = false;
                }
            }
        });
