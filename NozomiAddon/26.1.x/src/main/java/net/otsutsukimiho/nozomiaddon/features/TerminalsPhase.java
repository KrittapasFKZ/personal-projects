    ...
    public void initClient() {
        ...
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
            ...
            if (msg.contains("[BOSS] Storm: I should have known that I stood no chance") && !phaseStarted) {
                client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §9§lEntering Terminal Phase!"));
            ...
            if (matcher.find()) {
                ...
                if (current == totalTerminals && phaseStarted) {
                    if (currentSection <= 3) {
                        if (gateDestroy) {
                            client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §a§lSAFE! §d§lS§f§l" + currentSection));
                            customSound2.playTestSound();
                            titleText = Component.literal("§a§lSAFE!");
                            hideAt = System.currentTimeMillis() + 2000;
                        } else {
                            client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §c§lGATE NOT DESTROY! §d§lS§f§l" + currentSection));
            ...
            if (msg.contains("The Core entrance is opening!") && phaseStarted) {
                phaseStarted = false;
                client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §9§lTerminal Phase Completed!"));
            ...
            if (msg.contains("The gate has been destroyed!") && phaseStarted && !gateDestroy) {
                gateDestroy = true;
                client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §a§lGATE DESTROYED! §d§lS§f§l" + currentSection));
