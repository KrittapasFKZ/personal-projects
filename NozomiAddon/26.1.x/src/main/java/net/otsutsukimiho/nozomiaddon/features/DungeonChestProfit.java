...
public class DungeonChestProfit implements FeatureManager.Feature {
    ...
    private static Component hudText = Component.empty();
    ...
    private static final Map<String, Object> DROPS_MASTER_STAR = Map.ofEntries(
            Map.entry("FIRST_MASTER_STAR", SoundEvents.WITHER_DEATH),
            Map.entry("SECOND_MASTER_STAR", SoundEvents.WITHER_DEATH),
            Map.entry("THIRD_MASTER_STAR", SoundEvents.WITHER_DEATH),
            Map.entry("FOURTH_MASTER_STAR", SoundEvents.WITHER_DEATH),
            Map.entry("FIFTH_MASTER_STAR", SoundEvents.WITHER_DEATH)
    );

    private static final Map<String, Object> DROPS_RNG = Map.ofEntries(
            Map.entry("SHADOW_FURY", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("GIANTS_SWORD", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("PRECURSOR_EYE", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("SHADOW_WARP_SCROLL", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("IMPLOSION_SCROLL", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("WITHER_SHIELD_SCROLL", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("NECRON_HANDLE", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("DARK_CLAYMORE", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("DYE_LIVID", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("DYE_NECRON", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("ENCHANTMENT_THUNDERLORD_7", SoundEvents.ENDER_DRAGON_GROWL),
            Map.entry("MASTER_SKULL_TIER_5", SoundEvents.ENDER_DRAGON_GROWL)
    );

    private static final Map<String, Object> DROPS_NORMAL = Map.ofEntries(
            Map.entry("RECOMBOBULATOR_3000", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("BONZO_STAFF", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("SPIRIT_WING", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("SPIRIT_BONE", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("LIVID_DAGGER", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("SHADOW_ASSASSIN_CHESTPLATE", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("NECROMANCER_LORD_CHESTPLATE", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("SUMMONING_RING", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("WITHER_CHESTPLATE", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("ITEM_SPIRIT_BOW", SoundEvents.NOTE_BLOCK_PLING),
            Map.entry("LAST_BREATH", SoundEvents.NOTE_BLOCK_PLING)
    );
    ...
    public void initClient() {
        ClientTickEvents.END_CLIENT_TICK.register((client) -> {
            if (uiActive) {
                uiTicks++;
                if (uiTicks < 2) return;
                if (!(client.screen instanceof AbstractContainerScreen<?> handled)) return;
                if (croesusActive) {
                    croesusActive = false;
                    readCroesus(handled);
                }
                    ...
                    ScreenEvents.afterExtract(screen).register((screen1,drawContext, mouseX, mouseY, delta) -> {
                        ...
                        for (int i = 0; i < 19; i++) {
                            ...
                            for (Component line : lore.lines()) {
                                ...
                                if (cleanName.contains("Essence")) {
                                    ...
                                    if (getPrice == -1) {
                                        ...
                                        if (stack.getHoverName().getString().contains("the Fish")) {
                                            price += 0;
                                        } else {
                                ...
                                if (Minecraft.getInstance().player != null && finalId != null) {
                                    if (!processedSlots.contains(i)) {
                                        ...
                                        if (DROPS_NORMAL.containsKey(finalId)) {
                                            ...
                                            client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» ").append(chestName).append(line));
                                            if (DungeonChestProfit.showTitle.isEnabled()) {
                                                Minecraft.getInstance().gui.setTitle(Component.literal("").append(line).append(""));
                                                Minecraft.getInstance().gui.setSubtitle(Component.literal("§6§lRARE DROP"));
                                                Minecraft.getInstance().gui.setTimes(0, 50, 10);
                                            }
                                            hudText = hudText.copy().append(Component.literal("§d§k§lA§r ").append(line).append(" §d§k§lA§r "));
                                            announced = true;
                                            ...
                                            if (DungeonChestProfit.showTitle.isEnabled()) {
                                                Minecraft.getInstance().gui.setTitle(Component.literal("").append(line).append(""));
                                                Minecraft.getInstance().gui.setSubtitle(Component.literal("§d§lRNG DROP"));
                                                Minecraft.getInstance().gui.setTimes(0, 50, 10);
                                            }
                                            hudText = hudText.copy().append(Component.literal("§d§k§lA§r ").append(line).append(" §d§k§lA§r "));
                                            announced = true;
                            ...
                            if (alreadyOpen) {
                                ...
                                lines.add(Component.literal("§r").append(stack.getHoverName()).append(Component.literal("§f: §aAlready opened!")));
                            } else {
                                if (noMoreChest) {
                                    ChestProfitData data = new ChestProfitData(i, 0.0, false);
                                    chestsPrice.put(data.slot, data);
                                    lines.add(Component.literal("§r").append(stack.getHoverName()).append(Component.literal("§f: §cCan't open another chest!")));
                                } else {
                                    if (totalProfit >= 0) {
                                        ChestProfitData data = new ChestProfitData(i, totalProfit, true);
                                        chestsPrice.put(data.slot, data);
                                        lines.add(Component.literal("§r").append(stack.getHoverName()).append(Component.literal("§f: §a+" + shortNumber(totalProfit))));
                                    } else {
                                        ChestProfitData data = new ChestProfitData(i, 0.0, false);
                                        chestsPrice.put(data.slot, data);
                                        lines.add(Component.literal("§r").append(stack.getHoverName()).append(Component.literal("§f: §c-" + shortNumber(totalProfit * -1))));
                                    }
                                }
                            }
                        }
                        ...
                        int mainY = (Minecraft.getInstance().getWindow().getGuiScaledHeight() - 192) / 2 + 5;
                        int spacing = 10;

                        Component[] mainLines = lines.toArray(Component[]::new);
                        for (int i = 0; i < mainLines.length; i++) {
                            drawContext.text(Minecraft.getInstance().font, mainLines[i], mainX, mainY + (i * spacing), 0xFFFFFFFF, true);
                        }
                        ...
                        drawContext.text(
                                ...
                                hudText,
                                (screenWidth - Minecraft.getInstance().font.width(hudText)) / 2,
                                chestY - 15,
                                0xFFFFFFFF,
                                true
                        );
                    ...
                    if (titleStr.contains("Croesus")) {
                        ...
                        if (handler.slots.size() >= 29) {
                            ScreenEvents.afterExtract(screen).register((screen1, drawContext, mouseX, mouseY, delta) -> {
                                ...
                                if (slotChest.getItem().getHoverName().getString().contains("Open Reward Chest")) {
                                    var chestLore = slotChest.getItem().getComponents().get(DataComponents.LORE);
                                    if (chestLore != null) {
                                        var chestLines = chestLore.lines();
                                        for (Component line : chestLines) {
                                            String lineString = line.getString();
                                            String cleanName = removeFormat(lineString);
                                            if (cleanName.contains("Dungeon Chest Key")) {
                                                double keyPrice = PriceUtils.getBazaarBuyPrice("DUNGEON_CHEST_KEY");
                                        ...
                                        for (int i = 0; i < 29; i++) {
                                            ...
                                            if (cleanName.contains("Enchanted Book")) {
                                                if (lore != null && lore.lines().size() > 2) {
                                                    Component enchantLine = lore.lines().get(2);
                                                    ...
                                                    price += PriceUtils.getBazaarSellPrice(ID_From_Map);
                                                    lines.add(Component.literal("§r").append(enchantLine).append(String.format(" §6%s", shortNumber(price))));
                                                }
                                            } else {
                                                if (cleanName.contains("Essence")) {
                                                    ...
                                                    if (matcher.matches()) {
                                                        ...
                                                        int amount = ItemUtils.parseOptionalIntFromMatcher(matcher, "amount").orElse(1);
                                                        price += PriceUtils.getBazaarSellPrice(ID_From_Map) * amount;
                                                        lines.add(Component.literal("§r").append(stack.getHoverName()).append(String.format(" §6%s", shortNumber(price))));
                                                    }
                                                } else {
                                            ...
                                            if (Minecraft.getInstance().player != null) {
                                                if (!processedSlots.contains(i)) {
                                                    ...
                                                    if (DROPS_NORMAL.containsKey(finalId)) {
                                                        if (DungeonChestProfit.playSound.isEnabled()) customSound3.playTestSound();
                                                        client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» ").append(chestName).append(stack.getHoverName()));
                                                        if (DungeonChestProfit.showTitle.isEnabled()) {
                                                            Minecraft.getInstance().gui.setTitle(Component.literal("").append(stack.getHoverName()).append(""));
                                                            Minecraft.getInstance().gui.setSubtitle(Component.literal("§6§lRARE DROP"));
                                                            Minecraft.getInstance().gui.setTimes(0, 50, 10);
                                                        }
                                                        hudText = hudText.copy().append(Component.literal("§d§k§lA§r ")
                                        ...
                                        totalProfit -= chestPrice;
                                        lines.add(Component.literal("§r"));

                                        Component calProfit;
                                        if (totalProfit >= 0) {
                                            calProfit = Component.literal("§a+" + shortNumber(totalProfit) + " Coins");
                                            lines.add(Component.literal(String.format("§fProfit: §a+%s", shortNumber(totalProfit))));
                                        } else {
                                            calProfit = Component.literal("§c-" + shortNumber(totalProfit * -1) + " Coins");
                                            lines.add(Component.literal(String.format("§fProfit: §c-%s", shortNumber(totalProfit * -1))));
                                        }
                                        Component titleProfit = calProfit;

                                        int mainX = (screen1.width / 2) + 95;
                                        int mainY = (Minecraft.getInstance().getWindow().getGuiScaledHeight() - 192) / 2 - 10;
                                        int chestX = (Minecraft.getInstance().getWindow().getGuiScaledWidth() - 6) / 2;
                                        int textY = chestY - 10;
                                        int spacing = 10;
                                        Component[] mainLines = lines.toArray(Component[]::new);
                                        for (int i = 0; i < mainLines.length; i++) {
    ...
    public void readCroesus(AbstractContainerScreen<?> handled) {
        var handler = handled.getMenu();
        var slots = handler.slots;
        for (int i = 10; i < 45; i++) {
            var slot = slots.get(i);
