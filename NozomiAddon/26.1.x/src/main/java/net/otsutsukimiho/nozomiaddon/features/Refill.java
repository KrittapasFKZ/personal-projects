...
public class Refill {
    ...
    private static void triggerRefill() {
        ...
        if (!text.isEmpty()) {
            client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aRefilling " + text + "§a!"));
        } else {
            client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §cNo items to refill!"));
        }
    }
