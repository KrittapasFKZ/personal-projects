...
public class HideEquipment implements FeatureManager.Feature {
    ...
    public static KeySetting equipment1 = new KeySetting("Equipment #1", 49);
    public static KeySetting equipment2 = new KeySetting("Equipment #2", 50);
    public static KeySetting equipment3 = new KeySetting("Equipment #3", 51);
    public static KeySetting equipment4 = new KeySetting("Equipment #4", 52);
    public static KeySetting equipment5 = new KeySetting("Equipment #5", 53);
    public static KeySetting equipment6 = new KeySetting("Equipment #6", 54);
    public static KeySetting equipment7 = new KeySetting("Equipment #7", 55);
    public static KeySetting equipment8 = new KeySetting("Equipment #8", 56);
    public static KeySetting equipment9 = new KeySetting("Equipment #9", 57);
    ...
    public List<Settings> getSettings() {
        return List.of(enableKeyBind, blockReopen, safeUnequip, blockDuration, equipment1, equipment2, equipment3, equipment4, equipment5, equipment6, equipment7, equipment8, equipment9);
    }
        ...
        EventBus.register(HideWardrobe.class, PacketEvent.Receive.class, event -> {
            ...
            if (System.currentTimeMillis() < blockReopenUntil) {
                if (event.packet instanceof ClientboundOpenScreenPacket packet) {
                    if (packet.getTitle().getString().contains("Equipment Sets")) {
                        event.cancel();
                    }
                }
            }
        ...
        ScreenEvents.AFTER_INIT.register((Minecraft client, Screen screen, int scaledWidth, int scaledHeight) -> {
            ...
            ScreenMouseEvents.afterMouseClick(screen).register((screen1, mouseX, mouseY) -> {
                if (screen instanceof AbstractContainerScreen<?> handled) {
                    ...
                    if (titleStr.contains("Equipment Sets")) {
                        isWardrobe = true;
            ...
            ScreenKeyboardEvents.allowKeyPress(screen).register((scr, key) -> {
                ...
                if (!titleStr.contains("Equipment Sets")) return true;

                if (!HideEquipment.enableKeyBind.isEnabled()) return true;

                int code = key.input();
                int targetSlot = -1;

                if (code == equipment1.getCode()) targetSlot = 36;
                else if (code == equipment2.getCode()) targetSlot = 37;
                else if (code == equipment3.getCode()) targetSlot = 38;
                else if (code == equipment4.getCode()) targetSlot = 39;
                else if (code == equipment5.getCode()) targetSlot = 40;
                else if (code == equipment6.getCode()) targetSlot = 41;
                else if (code == equipment7.getCode()) targetSlot = 42;
                else if (code == equipment8.getCode()) targetSlot = 43;
                else if (code == equipment9.getCode()) targetSlot = 44;
                ...
                if (player != null && client.gameMode != null) {
                    ...
                    if (isUnequip && safeUnequip.isEnabled()) {
                        ...
                        if (targetSlot == lastUnequipSlot && (currentTime - lastUnequipTime) < blockDuration.getValue()) {
                            ...
                            client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §cPress again to unequip!"));
