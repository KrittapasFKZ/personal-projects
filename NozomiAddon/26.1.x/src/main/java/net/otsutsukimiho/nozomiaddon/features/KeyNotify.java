    ...
    public void initClient() {
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
            ...
            if (msg.contains("has obtained Blood Key!")) {
                client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §c§lBlood Key Picked!"));
                ...
                Minecraft.getInstance().gui.setSubtitle(formattedText);
                Minecraft.getInstance().gui.setTimes(0, 30, 10);
            }
            if (msg.contains("has obtained Wither Key!")) {
                client.gui.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §8§lWither Key Picked!"));
                customSound.playTestSound();
