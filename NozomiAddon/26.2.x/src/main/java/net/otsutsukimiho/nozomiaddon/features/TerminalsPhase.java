...
public class TerminalsPhase extends DraggableHudElement implements FeatureManager.Feature {
    ...
    public void initClient() {
        EventBus.register(TerminalsPhase.class, PacketEvent.Receive.class, event -> {
            ...
            if (event.packet instanceof ClientboundSetSubtitleTextPacket packet) {
        ...
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
            ...
            if (msg.contains("[BOSS] Storm: I should have known that I stood no chance") && !phaseStarted) {
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §9§lEntering Terminal Phase!"));
            ...
            if (matcher.find()) {
                ...
                if (current == totalTerminals && phaseStarted) {
                    if (currentSection <= 3) {
                        if (gateDestroy) {
                            client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §a§lSAFE! §d§lS§f§l" + currentSection));
                            customSound2.playTestSound();
                            titleText = Component.literal("§a§lSAFE!");
                            hideAt = System.currentTimeMillis() + 2000;
                        } else {
                            client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §c§lGATE NOT DESTROY! §d§lS§f§l" + currentSection));
                            customSound3.playTestSound();
                            titleText = Component.literal("§c§lGATE NOT DESTROY!");
                            hideAt = System.currentTimeMillis() + 2000;
                        }
                    }
                    currentSection++;
            ...
            if (msg.contains("The Core entrance is opening!") && phaseStarted) {
                phaseStarted = false;
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §9§lTerminal Phase Completed!"));
                customSound6.playTestSound();
                titleText = Component.literal("§f§l§kA§r §d§lCORE §f§l§kA");
                hideAt = System.currentTimeMillis() + 2000;
                return;
            }
            ...
            if (msg.contains("The gate has been destroyed!") && phaseStarted && !gateDestroy) {
                gateDestroy = true;
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §a§lGATE DESTROYED! §d§lS§f§l" + currentSection));
                customSound4.playTestSound();
                titleText = Component.literal("§a§lGATE DESTROYED!");
                hideAt = System.currentTimeMillis() + 2000;
            }
    ...
    public void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if ((System.currentTimeMillis() < hideAt) && TerminalsPhase.showGateTitle.isEnabled()) {
            ...
            ctx.pose().scale(TerminalsPhase.titleScale.getValue(), TerminalsPhase.titleScale.getValue());
        ...
        Component coloredMsg = ColorUtils.parseColor(newMsg);

        Component displayText;
        if (TerminalsPhase.showSection.isEnabled()) {
            displayText = Component.literal("§2§lS§f§l" + currentSection + " ").append(coloredMsg);
        } else {
