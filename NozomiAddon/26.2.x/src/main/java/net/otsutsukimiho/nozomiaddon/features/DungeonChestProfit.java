    ...
    public void initClient() {
        ClientTickEvents.END_CLIENT_TICK.register((client) -> {
            if (uiActive) {
                uiTicks++;
                if (uiTicks < 2) return;
                if (!(client.gui.screen() instanceof AbstractContainerScreen<?> handled)) return;
                if (croesusActive) {
                    croesusActive = false;
                    readCroesus(handled);
                }
                                ...
                                if (Minecraft.getInstance().player != null && finalId != null) {
                                    if (!processedSlots.contains(i)) {
                                        ...
                                        if (DROPS_NORMAL.containsKey(finalId)) {
                                            ...
                                            client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» ").append(chestName).append(line));
                                            if (DungeonChestProfit.showTitle.isEnabled()) {
                                                client.gui.hud.setTitle(Component.literal("").append(line).append(""));
                                                client.gui.hud.setSubtitle(Component.literal("§6§lRARE DROP"));
                                            ...
                                            if (DungeonChestProfit.showTitle.isEnabled()) {
                                                client.gui.hud.setTitle(Component.literal("").append(line).append(""));
                                                client.gui.hud.setSubtitle(Component.literal("§d§lRNG DROP"));
                                                client.gui.hud.setTimes(0, 50, 10);
                                            }
                                            hudText = hudText.copy().append(Component.literal("§d§k§lA§r ").append(line).append(" §d§k§lA§r "));
                                            announced = true;
                                        ...
                                        for (int i = 0; i < 29; i++) {
                                            ...
                                            if (Minecraft.getInstance().player != null) {
                                                if (!processedSlots.contains(i)) {
                                                    ...
                                                    if (DROPS_NORMAL.containsKey(finalId)) {
                                                        if (DungeonChestProfit.playSound.isEnabled()) customSound3.playTestSound();
                                                        client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» ").append(chestName).append(stack.getHoverName()));
                                                        if (DungeonChestProfit.showTitle.isEnabled()) {
                                                            client.gui.hud.setTitle(Component.literal("").append(stack.getHoverName()).append(""));
                                                            client.gui.hud.setSubtitle(Component.literal("§6§lRARE DROP"));
                                                            client.gui.hud.setTimes(0, 50, 10);
                                                        }
