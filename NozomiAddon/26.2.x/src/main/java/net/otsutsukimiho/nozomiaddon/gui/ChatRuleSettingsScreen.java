...
public class ChatRuleSettingsScreen extends Screen {
    ...
    public ChatRuleSettingsScreen(Screen parent) {
        super(Component.literal("Chat Rules Settings"));
        this.parent = parent;
    }

    private static class RuleUI {
        ChatRuleManager.ChatRule rule;
        EditBox triggerField;
        Button actionBtn;
        EditBox argField;
        Button deleteBtn;
    }
    ...
    protected void init() {
        ...
        addBtn = Button.builder(Component.literal("Add New Rule"), btn -> {
            ChatRuleManager.rules.add(new ChatRuleManager.ChatRule());
            ChatRuleManager.save();
            this.init(this.width, this.height);
        }).bounds(-1000, 0, usableWidth, 20).build();
        ...
        for (ChatRuleManager.ChatRule r : ChatRuleManager.rules) {
            ...
            ui.triggerField = new EditBox(font, -1000, 0, 200, 16, Component.literal(""));
            ui.triggerField.setMaxLength(256);
            ui.triggerField.setValue(r.triggerMessage);
            ui.triggerField.setResponder(val -> {
                r.triggerMessage = val;
                ChatRuleManager.save();
            });
            this.addRenderableWidget(ui.triggerField);

            ui.actionBtn = Button.builder(Component.literal(r.actionType), btn -> {
                activeDropdownRule = r;
            }).bounds(-1000, 0, 125, 20).build();
            this.addRenderableWidget(ui.actionBtn);

            ui.argField = new EditBox(font, -1000, 0, 150, 16, Component.literal(""));
            ui.argField.setMaxLength(256);
            ui.argField.setValue(r.actionArg);
            ui.argField.setResponder(val -> {
                if (r.actionType.equals("Hide Players X seconds") && !val.matches("\\d*")) {
                    String numbersOnly = val.replaceAll("[^\\d]", "");
                    ui.argField.setValue(numbersOnly);
            ...
            this.addRenderableWidget(ui.argField);
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.centeredText(this.font, "§nChat Rules", mainX + (mainWidth / 2), 15, 0xFFFFFFFF);
        ...
        for (RuleUI ui : uiElements) {
            if (currentY + 50 > mainY && currentY < mainY + mainHeight) {
                ...
                if (ui.triggerField.getValue().isEmpty() && !ui.triggerField.isFocused()) {
                    context.text(this.font, "Trigger Message / Regex", elementX + 4, currentY + 4, 0x777777, false);
                }
                if (ui.argField.getValue().isEmpty() && !ui.argField.isFocused()) {
                    String argHint = "Args / Command (e.g. &cAlert)";
                    if (ui.rule.actionType.equals("Hide Players X seconds")) argHint = "Seconds (Int)";

                    context.text(this.font, argHint, elementX + 134, currentY + 28, 0x777777, false);
                }
        ...
        if (activeDropdownRule != null) {
            ...
            for (String act : ACTION_TYPES) {
                ...
                context.centeredText(this.font, act, dropdownX + (dropdownWidth / 2), drawY + 3, isHovered ? 0xFFFFFFFF : 0xFFAAAAAA);
                drawY += 14;
            }
        }
    }
