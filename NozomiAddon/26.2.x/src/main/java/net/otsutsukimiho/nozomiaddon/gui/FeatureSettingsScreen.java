...
public class FeatureSettingsScreen extends Screen {
    ...
    private final Map<NumberSetting, EditBox> numberFields = new HashMap<>();
    private final Map<ColorSetting, EditBox[]> colorFields = new HashMap<>();
    private final Map<StringSetting, EditBox> stringFields = new HashMap<>();
    private final Map<FloatSetting, EditBox> floatFields = new HashMap<>();
    private final Map<BooleanSetting, Button> booleanButtons = new HashMap<>();
    private final Map<ModeSetting, Button> modeButtons = new HashMap<>();
    private final Map<KeySetting, Button> keyButtons = new HashMap<>();
    private final Map<RangeSetting, EditBox[]> rangeFields = new HashMap<>();
    private final Map<TextSetting, EditBox> textFields = new HashMap<>();
    private final Map<TagSetting, EditBox> tagFields = new HashMap<>();

    public FeatureSettingsScreen(Screen parent, String featureName, FeatureManager.Feature feature) {
        super(Component.literal("Settings: " + featureName));
        this.parent = parent;
        ...
        for (Settings setting : feature.getSettings()) {
            ...
            if (setting instanceof BooleanSetting bool) {
                Button btn = Button.builder(
                        Component.literal(bool.isEnabled() ? "§aEnabled" : "§cDisabled"),
                        b -> {
                            bool.toggle();
                            b.setMessage(Component.literal(bool.isEnabled() ? "§aEnabled" : "§cDisabled"));
                            ConfigManager.save();
                        }
                ).bounds(dummyX, 0, WIDGET_WIDTH + FIELD_WIDTH + 5, 20).build();
                this.addRenderableWidget(btn);
                booleanButtons.put(bool, btn);
            }
            else if (setting instanceof StringSetting str) {
                EditBox field = new EditBox(font, dummyX, 0, WIDGET_WIDTH + FIELD_WIDTH + 5, 16, Component.literal(""));
                field.setMaxLength(256);
                field.setValue(str.getValue());
                ...
                EditBox field = new EditBox(font, dummyX, 0, WIDGET_WIDTH + FIELD_WIDTH + 5, 16, Component.literal(""));
                field.setMaxLength(256);
                field.setValue(textSetting.getValue());
                field.setResponder(val -> {
                ...
                Button btn = Button.builder(
                        Component.literal(mode.getMode()),
                        b -> {
                            mode.cycle();
                            b.setMessage(Component.literal(mode.getMode()));
                            ConfigManager.save();
                        }
                ).bounds(dummyX, 0, WIDGET_WIDTH + FIELD_WIDTH + 5, 20).build();
                this.addRenderableWidget(btn);
                ...
                field.setValue(tagSetting.getTagsAsString());
                field.setResponder(val -> {
                ...
                EditBox r = createColorField(dummyX, 0, color, "R");
                EditBox g = createColorField(dummyX, 0, color, "G");
                EditBox b = createColorField(dummyX, 0, color, "B");

                r.setValue(String.valueOf(c.getRed()));
                g.setValue(String.valueOf(c.getGreen()));
                b.setValue(String.valueOf(c.getBlue()));

                colorFields.put(color, new EditBox[]{r, g, b});
            }
            else if (setting instanceof NumberSetting num) {
                EditBox field = new EditBox(font, dummyX, 0, FIELD_WIDTH, 14, Component.literal(""));
                field.setValue(String.valueOf(num.getValue()));
                field.setResponder(val -> {
                ...
                EditBox field = new EditBox(font, dummyX, 0, FIELD_WIDTH, 14, Component.literal(""));
                field.setValue(String.format("%.2f", flt.getValue()));
                field.setResponder(val -> {
                    if (val.isEmpty()) return;
                    try {
                ...
                Button btn = Button.builder(Component.literal(btnText), b -> {
                    bindingKey = key;
                ...
                EditBox minField = new EditBox(font, dummyX, 0, 30, 14, Component.literal(""));
                minField.setValue(String.valueOf(rs.getMin()));
                minField.setResponder(val -> {
                    if (val.isEmpty()) return;
                    try { rs.setMin(Integer.parseInt(val)); ConfigManager.save(); } catch (NumberFormatException ignored) {}
                });

                EditBox maxField = new EditBox(font, dummyX, 0, 30, 14, Component.literal(""));
                maxField.setValue(String.valueOf(rs.getMax()));
                maxField.setResponder(val -> {
                    if (val.isEmpty()) return;
                ...
                rangeFields.put(rs, new EditBox[]{minField, maxField});
            }
        }
    ...
    private EditBox createColorField(int x, int y, ColorSetting setting, String type) {
        EditBox field = new EditBox(font, x, y, 35, 14, Component.literal(""));
        field.setMaxLength(3);
        field.setResponder(val -> {
            if (val.isEmpty()) return;
            try {
                int i = Mth.clamp(Integer.parseInt(val), 0, 255);
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.centeredText(this.font, "§n" + featureName + " Settings", mainX + (mainWidth / 2), 15, 0xFFFFFFFF);
        ...
        for (Settings setting : feature.getSettings()) {
            ...
            if (currentY + settingHeight > mainY && currentY < mainY + mainHeight) {
                context.text(font, setting.name, labelX, currentY + 6, 0xFFFFFFFF);
                ...
                if (setting instanceof BooleanSetting bool) {
                    ...
                    EditBox field = tagFields.get(tagSetting);
                    if (field != null) {
                    ...
                    for (String tag : tagSetting.getTags()) {
                        ...
                        context.text(font, tag, pillX + 4, pillY + 2, 0xFFFFFFFF, false);
                        pillX += tagWidth + 4;
                    }
                }
                    ...
                    context.centeredText(font, "Select (" + multi.getSelectedOptions().size() + ")", widgetX + (dWidth / 2), currentY + 4, 0xFFFFFFFF);
                    context.text(font, "v", widgetX + dWidth - 10, currentY + 4, 0xFFAAAAAA);
                    ...
                    if (check.isEnabled()) {
                        ...
                        context.text(font, "✔", boxX + 3, boxY + 1, 0xFFFFFFFF);
                    } else {
                        context.fill(boxX + 1, boxY + 1, boxX + boxSize - 1, boxY + boxSize - 1, 0xFF111111);
                    }
                    int textWidth = font.width(textStr);
                    context.text(font, textStr, boxX - textWidth - 8, currentY + 6, textColor);
                }
                else if (setting instanceof TextSetting textSetting) {
                    EditBox field = textFields.get(textSetting);
                    ...
                    Component coloredPreview = ColorUtils.parseColor(preview);
                    context.text(font, coloredPreview, widgetX, currentY + 22, 0xFFAAAAAA, true);
                }
                else if (setting instanceof ModeSetting mode) {
                    int dWidth = WIDGET_WIDTH + FIELD_WIDTH + 5;
                    context.fill(widgetX, currentY, widgetX + dWidth, currentY + 15, 0xFF333333);
                    context.centeredText(font, mode.getMode(), widgetX + (dWidth / 2), currentY + 4, 0xFFFFFFFF);
                    context.text(font, "v", widgetX + dWidth - 10, currentY + 4, 0xFFAAAAAA);
                    if (activeModeDropdown == mode) { dropdownX = widgetX; dropdownY = currentY + 15; }
                }
                else if (setting instanceof StringSetting str) {
                    EditBox field = stringFields.get(str);
                    if (field != null) { field.setX(widgetX); field.setY(currentY + 2); field.visible = true; }
                }
                else if (setting instanceof NumberSetting num) {
                    drawNumberSlider(context, num, widgetX, currentY + 6, mouseX, mouseY);
                    EditBox field = numberFields.get(num);
                    ...
                    EditBox field = floatFields.get(flt);
                    if (field != null) { field.setX(widgetX + WIDGET_WIDTH + 5); field.setY(currentY + 3); field.visible = true; }
                }
                else if (setting instanceof RangeSetting rs) {
                    drawRangeSlider(context, rs, widgetX + 35, currentY + 6, 75, mouseX, mouseY);
                    EditBox[] fields = rangeFields.get(rs);
                    if (fields != null) {
                    ...
                    EditBox[] fields = colorFields.get(color);
                    if (fields != null) {
                        ...
                        for (EditBox f : fields) f.visible = true;
                    }
                }
                else if (setting instanceof KeySetting key) {
                    Button btn = keyButtons.get(key);
                    if (btn != null) {
                        btn.setX(widgetX); btn.setY(currentY);
                        if (bindingKey == key) btn.setMessage(Component.literal("..."));
                        else btn.setMessage(Component.literal(key.getKeyName()));
                        btn.visible = true;
                    }
                }
                    ...
                    context.text(font, display, widgetX + 4, currentY + 4, 0xFFFFFFFF);
                    context.text(font, "v", widgetX + dWidth - 10, currentY + 4, 0xFFAAAAAA);

                    int sliderX = widgetX + 55; int sliderW = dWidth - 60;
                    int volY = currentY + 20;
                    context.text(font, "Vol: " + String.format("%.2f", sound.getVolume()), widgetX, volY - 2, 0xFFFFFFFF);
                    ...
                    context.text(font, "Pitch: " + String.format("%.1f", sound.getPitch()), widgetX, pitchY - 2, 0xFFFFFFFF);
                    ...
                    context.centeredText(font, "Play Test Sound", widgetX + (dWidth / 2), playY + 2, 0xFFFFFFFF);
        ...
        if (activeDropdown != null) {
            ...
            dropdownScrollIndex = Mth.clamp(dropdownScrollIndex, 0, maxDropdownScroll);
            ...
            context.text(this.font, searchStr + "_", dropdownX + 4, dropdownY + 4, 0xFFFFFFFF, false);
            ...
            for (int i = dropdownScrollIndex; i < maxIndex; i++) {
                ...
                context.text(this.font, display, dropdownX + 4, drawY + 1, isHovered ? 0xFFFFFFFF : 0xFFAAAAAA, false);
                drawY += itemHeight;
            }
        ...
        if (activeModeDropdown != null) {
            ...
            for (String modeStr : modes) {
                ...
                context.centeredText(this.font, modeStr, dropdownX + (dropdownWidth / 2), drawY + 1, isHovered ? 0xFFFFFFFF : 0xFFAAAAAA);
                drawY += 10;
            }
        }
        ...
        if (activeMultiDropdown != null) {
            ...
            multiScrollIndex = Mth.clamp(multiScrollIndex, 0, maxMultiScroll);
            ...
            for (int i = multiScrollIndex; i < maxIndex; i++) {
                ...
                boolean isHovered = mouseX >= dropdownX && mouseX <= dropdownX + dropdownWidth - 6 && mouseY >= drawY && mouseY <= drawY + itemHeight;
                if (isHovered) context.fill(dropdownX + 1, drawY, dropdownX + dropdownWidth - 6, drawY + itemHeight, 0xFF44AAFF);
                if (activeMultiDropdown.isSelected(opt)) context.text(font, "✔", dropdownX + 4, drawY + 1, 0xFF55FF55);
                context.text(font, opt, dropdownX + 16, drawY + 1, isHovered ? 0xFFFFFFFF : 0xFFAAAAAA);
                drawY += itemHeight;
            }
    ...
    private void hideWidgetsFor(Settings setting) {
        ...
        else if (setting instanceof ColorSetting && colorFields.containsKey(setting)) for (EditBox f : colorFields.get(setting)) f.visible = false;
        ...
        else if (setting instanceof RangeSetting && rangeFields.containsKey(setting)) for (EditBox f : rangeFields.get(setting)) f.visible = false;
    }

    private void drawRangeSlider(GuiGraphicsExtractor context, RangeSetting setting, int x, int y, int width, int mouseX, int mouseY) {
        ...
        if (draggingRangeSetting == setting) {
            ...
            rangeFields.get(setting)[0].setValue(String.valueOf(setting.getMin()));
            rangeFields.get(setting)[1].setValue(String.valueOf(setting.getMax()));
            ConfigManager.save();
        }
    ...
    private void drawColorSection(GuiGraphicsExtractor context, ColorSetting setting, int x, int y, int mouseX, int mouseY) {
    ...
    private void drawSingleColorSlider(GuiGraphicsExtractor context, ColorSetting setting, String type, int value, int x, int y, int color, int mouseX) {
        context.text(font, type, x - 10, y - 2, 0xFFFFFFFF, true);
        ...
        if (draggingColor == setting && draggingColorComponent.equals(type)) {
            ...
            int newVal = (int)(Mth.clamp(percent, 0, 1) * 255);
            updateColorFromField(setting, type, newVal);

            EditBox[] fields = colorFields.get(setting);
            if (fields != null) {
                if (type.equals("R")) fields[0].setValue(String.valueOf(newVal));
                if (type.equals("G")) fields[1].setValue(String.valueOf(newVal));
                if (type.equals("B")) fields[2].setValue(String.valueOf(newVal));
            }
            ConfigManager.save();
            value = newVal;
        }
    ...
    private void drawFloatSlider(GuiGraphicsExtractor context, FloatSetting setting, int x, int y, int mouseX, int mouseY) {
        ...
        if (draggingFloat == setting) {
            ...
            if (newVal != setting.getValue()) {
                ...
                ConfigManager.save();
                if (floatFields.containsKey(setting)) floatFields.get(setting).setValue(String.format("%.2f", setting.getValue()));
            }
        }
    ...
    private void drawNumberSlider(GuiGraphicsExtractor context, NumberSetting setting, int x, int y, int mouseX, int mouseY) {
        context.fill(x, y, x + WIDGET_WIDTH, y + 4, 0xFF444444);
        ...
        if (draggingSetting == setting) {
            ...
            if (newVal != setting.getValue()) {
                setting.setValue(newVal);
                ConfigManager.save();
                if (numberFields.containsKey(setting)) numberFields.get(setting).setValue(String.valueOf(setting.getValue()));
            }
        }
    ...
    public boolean mouseClicked(MouseButtonEvent click, boolean doubled) {
        ...
        if (bindingKey != null) {
            ...
            Button btn = keyButtons.get(bindingKey);
            if (btn != null) btn.setMessage(Component.literal(bindingKey.getKeyName()));
            ConfigManager.save();
            bindingKey = null;
            return true;
        }
