...
public class DungeonRunOverview extends DraggableHudElement implements FeatureManager.Feature {
    ...
    public void initClient() {
        ...
        ClientReceiveMessageEvents.GAME.register((message, overlay) -> {
            ...
            if (msg.contains("[BOSS] The Watcher: That will be enough for now.")) {
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §cBlood Ready!"));
            }
            ...
            if (msg.contains("[BOSS] Storm: Pathetic Maxor, just like expected.")) {
                if (dungeonStep == 3) {
                    dungeonStep = 4;
                    client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §aMaxor Done: %.2fs", (ticks.getOrDefault("Maxor", 0F) / 20.0))));
                }
                zone = "Storm";
            }
            if (msg.contains("[BOSS] Goldor: Who dares trespass into my domain?")) {
                if (dungeonStep == 4) {
                    dungeonStep = 5;
                    client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §bStorm Done: %.2fs", (ticks.getOrDefault("Storm", 0F) / 20.0))));
                }
                zone = "Terminals";
            }
            if (msg.contains("The Core entrance is opening!")) {
                if (dungeonStep == 5) {
                    dungeonStep = 6;
                    client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §eTerminals Done: %.2fs", (ticks.getOrDefault("Terminals", 0F) / 20.0))));
                }
                zone = "Goldor";
            }
            if (msg.contains("[BOSS] Necron: You went further than any human before, congratulations.")) {
                if (dungeonStep == 6) {
                    dungeonStep = 7;
                    client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §7Goldor Done: %.2fs", (ticks.getOrDefault("Goldor", 0F) / 20.0))));
                }
                zone = "Necron";
            }
            if (msg.contains("[BOSS] Necron: All this, for nothing...")) {
                if (dungeonStep == 7) {
                    dungeonStep = 8;
                    client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §cNecron Done: %.2fs", (ticks.getOrDefault("Necron", 0F) / 20.0))));
                }
                zone = "Dragons";
            }
        });
    ...
    public static void tick () {
        ...
        if (bloodStart) {
            ...
            if (dungeonStep == 0) {
                dungeonStep = 1;
                client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §c§lBLOOD DOOR §chas been opened! §f(§a%s§f)", bloodOpened)));
            }
        }
        if (DUNGEON.watcherCleared != -1 && DUNGEON.bossEntry == -1) {
            ...
            if (dungeonStep == 1) {
                dungeonStep = 2;
                client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §cBlood Done! §f(took §a%.2fs§f)", (bloodTick / 20.0))));
            }
        }
        if (DUNGEON.bossEntry != -1) {
            ...
            if (dungeonStep == 2) {
                dungeonStep = 3;
                client.gui.hud.getChat().addClientSystemMessage(Component.literal(String.format("§d§lNA §f§l» §aBoss Entry: §a%s", getTime(bossEntry))));
