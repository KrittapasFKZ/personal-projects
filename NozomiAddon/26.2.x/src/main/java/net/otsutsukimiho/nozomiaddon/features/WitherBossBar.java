...
public class WitherBossBar extends DraggableHudElement implements FeatureManager.Feature {
    ...
    public void initClient() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(c -> {
            ...
            BossHealthOverlay hud = client.gui.hud.getBossOverlay();
            ...
            for (LerpingBossEvent bar : bars.values()) {
                ...
                if (title.contains(currentBoss)) {
                    ...
                    if (bossInitialized && !bossDeadAnnounced && percent <= 0.0f) {
                        ...
                        titleText = Component.literal("§4§l" + currentBoss + " §cDIED!");
                        hideAt = System.currentTimeMillis() + 2000;

                        if (WitherBossBar.announceDead.isEnabled() && client.player != null) {
                            client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §4§l" + currentBoss + " §chas died!"));
                        }
                    }
                }
            }
    ...
    private void switchBoss(String bossName) {
        ...
        Minecraft client = Minecraft.getInstance();
        if (client.player != null) client.gui.hud.getChat().addClientSystemMessage(Component.nullToEmpty("§d§lNA §f§l» §4§l" + bossName + " §chas spawned!"));
    }
