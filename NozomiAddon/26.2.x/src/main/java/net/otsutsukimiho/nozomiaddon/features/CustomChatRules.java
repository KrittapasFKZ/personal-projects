...
public class CustomChatRules implements FeatureManager.Feature {
    ...
    private void executeAction(Minecraft client, ChatRuleManager.ChatRule rule, String originalMessage) {
        ...
        switch (rule.actionType) {
            case "Alert Title":
                client.gui.hud.setTitle(parsedArg.isEmpty() ? Component.empty() : Component.literal(parsedArg));
                client.gui.hud.setSubtitle(Component.empty());
                client.gui.hud.setTimes(0, 30, 10);
                break;
            case "Alert Subtitle":
                client.gui.hud.setTitle(Component.empty());
                client.gui.hud.setSubtitle(Component.literal(parsedArg));
                client.gui.hud.setTimes(0, 30, 10);
                break;
            case "Alert Message":
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» " + parsedArg));
                break;
            case "Run Command":
