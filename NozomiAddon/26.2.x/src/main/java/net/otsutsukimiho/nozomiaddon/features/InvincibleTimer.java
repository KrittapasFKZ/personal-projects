...
public class InvincibleTimer extends DraggableHudElement implements FeatureManager.Feature {
    ...
    public void render(GuiGraphicsExtractor ctx, DeltaTracker tickCounter) {
        ...
        if (EditHudScreen.isEditMode()) {
            ...
            ctx.text(Minecraft.getInstance().font, text, centerX, 0, 0xFFFFFFFF, true);
        }
        ...
        if (spiritTime > 0) {
            ...
            int textWidth = Minecraft.getInstance().font.width(text);
            int centerX = 25 - textWidth / 2;
            ctx.text(Minecraft.getInstance().font, Component.literal(text), centerX, y, 0xFFFFFFFF, true);
            y += 12;
        }
        if (bonzoTime > 0) {
