...
public class LeapMenu implements FeatureManager.Feature {
    ...
    public void initClient() {
                ...
                if (title.equals("Spirit Leap") || title.equals("Teleport to Player")) {
                    ...
                    ScreenEvents.afterExtract(screen).register((screen1, drawContext, mouseX, mouseY, delta) -> {
                        ...
                        for (int i = 0; i < players.size() && i < 4; i++) {
                            ...
                            boxScales[quadrantIndex] = Mth.lerp(0.25f * tickDelta, boxScales[quadrantIndex], targetAnimScale);
                            ...
                            drawContext.pose().scale(finalScale, finalScale);
                            ...
                            if (player.skin != null) {
                                drawContext.blit(RenderPipelines.GUI_TEXTURED, player.skin, faceX, faceY, 8f, 8f, faceSize, faceSize, 8, 8, 64, 64);
                            }

                            int textX = faceX + faceSize + 8;
                            String playerName = "§f" + player.name;
                            drawContext.text(client.font, Component.literal(playerName), textX, faceY + 4, 0xFFFFFFFF);
                            String className = colorStyle.isEnabled() ? "§7" + player.dClass.displayName.substring(2) : player.dClass.displayName;
                            String classText = player.isDead ? "§c§lDEAD" : className;
                            drawContext.text(client.font, Component.literal(classText), textX, faceY + 16, 0xFFFFFF55);
                    ...
                    ScreenMouseEvents.allowMouseClick(screen).register((screen2, click) -> {
                        ...
                        int halfW = client.getWindow().getGuiScaledWidth() / 2;
                        int halfH = client.getWindow().getGuiScaledHeight() / 2;
                        ...
                        if (quadrant < players.size()) {
                            ...
                            if (target.isDead) {
                                if (client.player != null) client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §cThis player is dead!"));
                                return false;
                            }

                            if (client.gameMode != null && client.player != null) {
                                client.gameMode.handleContainerInput(handled.getMenu().containerId, target.slotIndex, 0, ContainerInput.PICKUP, client.player);
                                client.player.containerMenu.setCarried(ItemStack.EMPTY);
                                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aLeaping to " + target.name + "!"));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (isOpen) {
                if (!(client.gui.screen() instanceof AbstractContainerScreen<?> handled) || (!handled.getTitle().getString().equals("Spirit Leap") && !handled.getTitle().getString().equals("Teleport to Player"))) {
                    isOpen = false;
                }
            }
        });
    ...
    private static void drawRoundedGradientRect(GuiGraphicsExtractor context, int x, int y, int colorTop, int colorBottom) {
    ...
    private List<LeapPlayer> parseLeapPlayers(AbstractContainerScreen<?> handled, Minecraft client) {
        ...
        if (client.player != null) {
            for (PlayerInfo entry : client.player.connection.getOnlinePlayers()) {
                if (entry.getTabListDisplayName() != null) {
                    String tabName = entry.getTabListDisplayName().getString();
        ...
        for (int i = 11; i <= 16; i++) {
            ...
            if (!stack.isEmpty() && stack.getItem() == Items.PLAYER_HEAD) {
                ...
                String cleanName = rawName.replaceAll("(?i)[§&][0-9a-zA-Z]", "").trim();
                String noRankName = cleanName.replaceAll("\\[.*?\\]\\s*", "").trim();
                String pureName = noRankName.replaceAll("[^a-zA-Z0-9_]", "");
                ...
                players.add(new LeapPlayer(cleanName, pureName, dClass, isDead, skin, stack, i));
            }
        }
        ...
        if (currentMode.equals("Odin Sorting")) {
            ...
            }).thenComparing(p -> p.pureName.toLowerCase()));

        } else if (currentMode.equals("A-Z Class")) {
            players.sort(Comparator.comparing((LeapPlayer p) -> p.dClass.name().toLowerCase())
                    .thenComparing(p -> p.pureName.toLowerCase()));

        } else {
            players.sort(Comparator.comparing(p -> p.pureName.toLowerCase()));
        }
    ...
    private record LeapPlayer(String name, String pureName, DungeonClass dClass, boolean isDead, net.minecraft.resources.Identifier skin, ItemStack head, int slotIndex) {}
    ...
    private enum DungeonClass {
        ...
        DungeonClass(String displayName, int colorCode) {
