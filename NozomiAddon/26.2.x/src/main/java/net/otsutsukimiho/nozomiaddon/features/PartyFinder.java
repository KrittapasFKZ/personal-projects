...
public class PartyFinder extends DraggableHudElement implements FeatureManager.Feature {
    ...
    public void initClient() {
        ...
        ClientReceiveMessageEvents.ALLOW_GAME.register((message, overlay) -> {
            ...
            if (msg.contains("Party Finder > Your party has been queued in the dungeon finder") && !partyQueued) {
                ...
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aQueued to §dParty Finder§a!"));
                if (PartyFinder.queueAddonShowTitle.isEnabled()) {
                    client.gui.hud.setTitle(Component.literal("§a§lQueued!"));
            ...
            if (matcher_partyLeave.find() && matcher_partyLeave.group(1) != null && partyQueued) {
                ...
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §e" + player_name + " §cleft! §e" + partyAmount + "§f/§a5"));
                if (PartyFinder.queueAddonShowTitle.isEnabled()) {
                    client.gui.hud.setTitle(Component.literal("§e§l" + partyAmount + "§f§l/§a§l5"));
                    client.gui.hud.setSubtitle(Component.literal("§e" + player_name + " §cleft!"));
                    client.gui.hud.setTimes(0,30,5);
                }
                hideMessage = true;
            }
            ...
            if (isPfJoin && matcher_pfJoin.group(1) != null && matcher_pfJoin.group(2) != null) {
                if (partyQueued && partyAmount <= 4) {
                    ...
                    client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §e" + player_name + " " + class_level_name + " §ajoined! §e" + partyAmount + "§f/§a5"));
                    if (PartyFinder.queueAddonShowTitle.isEnabled()) {
                        client.gui.hud.setTitle(Component.literal("§e§l" + partyAmount + "§f§l/§a§l5"));
                        client.gui.hud.setSubtitle(Component.literal("§e" + player_name + " §ajoined!"));
                        client.gui.hud.setTimes(0,30,5);
                    }
                } else if (!partyQueued) {
                    partyQueued = true;
                    ...
                    String class_level_name = CLASS_COLORS.get(class_name) + " §8(" + getLevelColor(class_level) + class_level + "§8)";
                    client.player.connection.sendCommand("pl");
                    client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aJoined §dParty Finder§a! " + class_level_name));
                    if (PartyFinder.queueAddonShowTitle.isEnabled()) {
                        client.gui.hud.setTitle(Component.literal("§a§lJoined!"));
                        client.gui.hud.setSubtitle(Component.literal(" "));
                        client.gui.hud.setTimes(0,30,5);
                    }
                }
            ...
            if (matcher_partyJoin.find() && matcher_partyJoin.group(1) != null && partyQueued && partyAmount <= 4) {
                ...
                partyAmount += 1;
                customSound4.playTestSound();
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §e" + player_name + " §ajoined! §e" + partyAmount + "§f/§a5"));
                if (PartyFinder.queueAddonShowTitle.isEnabled()) {
                    client.gui.hud.setTitle(Component.literal("§e§l" + partyAmount + "§f§l/§a§l5"));
                    client.gui.hud.setSubtitle(Component.literal("§e" + player_name + " §ajoined!"));
                    client.gui.hud.setTimes(0,30,5);
            ...
            if (matcher_partyMembers.find() && matcher_partyMembers.group(1) != null && partyQueued && !debounce_partyMembers) {
                ...
                partyAmount = amount;
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aCurrent Party Members: §e" + amount + "§f/§a5"));
                hideMessage = true;
            }
            ...
            if (delistAlert != null && partyQueued) {
                ...
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aQueued for §b" + getTime(System.currentTimeMillis() - queuedTime)));
                client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» " + delistAlert));
                if (PartyFinder.queueAddonShowTitle.isEnabled()) {
                    client.gui.hud.setTitle(Component.literal("§c§lDe-listed!"));
                    client.gui.hud.setSubtitle(Component.literal(" "));
                    client.gui.hud.setTimes(0,30,5);
                }
                hideMessage = true;
    ...
    private void checkFullParty() {
        ...
        if (partyQueued && partyAmount >= 5) {
            ...
            customSound2.playTestSound();
            client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aQueued for §b" + getTime(System.currentTimeMillis() - queuedTime)));
            client.gui.hud.getChat().addClientSystemMessage(Component.literal("§d§lNA §f§l» §aDe-listed! §6§lParty Full! (5/5)"));
            if (PartyFinder.queueAddonShowTitle.isEnabled()) {
                client.gui.hud.setTitle(Component.literal("§6§lPARTY FULL! (5/5)"));
                client.gui.hud.setSubtitle(Component.literal(" "));
                client.gui.hud.setTimes(0,40,5);
            }
